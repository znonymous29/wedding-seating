...
// 获取 Socket URL
// 生产环境：使用当前页面的 origin（通过 nginx 代理 /socket.io 到后端）
// 开发环境：使用环境变量或默认的 localhost:3001
const getSocketUrl = (): string => {
  const envUrl = import.meta.env.VITE_SOCKET_URL;
  if (envUrl) {
    return envUrl;
  }
  // 生产环境下使用当前域名（nginx 会代理 /socket.io）
...
export const connectSocket = (): Socket => {
  ...
  socket = io(socketUrl, {
    ...
    transports: ["websocket", "polling"], // 添加 polling 作为后备
    reconnection: true,
...
export const joinProject = (projectId: string) => {
  ...
  if (socket?.connected) {
    ...
    console.log(
      "Socket not connected yet, will join project on connect:",
      projectId
    );
  }
};
...
export const setupSocketListeners = (handlers: SocketEventHandlers) => {
  ...
  if (handlers.onMemberOnline)
    socket.on("member:online", handlers.onMemberOnline);
  if (handlers.onMemberOffline)
    socket.on("member:offline", handlers.onMemberOffline);

  // 宾客事件
  if (handlers.onGuestCreated)
    socket.on("guest:created", handlers.onGuestCreated);
  if (handlers.onGuestUpdated)
    socket.on("guest:updated", handlers.onGuestUpdated);
  if (handlers.onGuestDeleted)
    socket.on("guest:deleted", handlers.onGuestDeleted);
  if (handlers.onGuestsImported)
    socket.on("guests:imported", handlers.onGuestsImported);
  if (handlers.onGuestsDeleted)
    socket.on("guests:deleted", handlers.onGuestsDeleted);

  // 桌位事件
  if (handlers.onTableCreated)
    socket.on("table:created", handlers.onTableCreated);
  if (handlers.onTableUpdated)
    socket.on("table:updated", handlers.onTableUpdated);
  if (handlers.onTableDeleted)
    socket.on("table:deleted", handlers.onTableDeleted);
  if (handlers.onTablesCreated)
    socket.on("tables:created", handlers.onTablesCreated);
  if (handlers.onTablePositionsUpdated)
    socket.on("tables:positions-updated", handlers.onTablePositionsUpdated);

  // 座位安排事件
  if (handlers.onSeatingAssigned)
    socket.on("seating:assigned", handlers.onSeatingAssigned);
  if (handlers.onSeatingUnassigned)
    socket.on("seating:unassigned", handlers.onSeatingUnassigned);
  if (handlers.onSeatingMoved)
    socket.on("seating:moved", handlers.onSeatingMoved);
  if (handlers.onSeatingAutoAssigned)
    socket.on("seating:auto-assigned", handlers.onSeatingAutoAssigned);

  // 编辑锁定事件
  if (handlers.onEditingLocked)
    socket.on("editing:locked", handlers.onEditingLocked);
  if (handlers.onEditingUnlocked)
    socket.on("editing:unlocked", handlers.onEditingUnlocked);
