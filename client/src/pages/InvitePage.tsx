...
import { Button, Card, Spin, Result, message } from 'antd'
...
export default function InvitePage() {
  ...
  const handleJoin = async () => {
    ...
    try {
      ...
      message.error(error.response?.data?.message || '加入失败，邀请链接可能已过期')
    } finally {
      setLoading(false)
    }
  }
  ...
  if (joined && projectInfo) {
    return (
      <div className={styles.container}>
        <Card className={styles.card} style={{ maxWidth: 400, textAlign: 'center' }}>
          <Result
  ...
  return (
    <div className={styles.container}>
      <Card className={styles.card} style={{ maxWidth: 400, textAlign: 'center', padding: '32px' }}>
